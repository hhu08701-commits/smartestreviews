...
<div class="max-w-7xl mx-auto">
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-900" style="font-family: 'Montserrat', sans-serif;">Edit Post</h2>
    </div>
    ...
    <form method="POST" action="{{ route('admin.posts.update', $post) }}" enctype="multipart/form-data" id="post-form">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <!-- Slug -->
                <div class="bg-white shadow rounded-lg p-6">
                    <label for="slug" class="block text-sm font-medium text-gray-700 mb-2">Permalink</label>
                    <div class="flex items-center space-x-2">
                        <span class="text-sm text-gray-500">/posts/</span>
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $post->slug) }}"
                               class="flex-1 border-gray-300 rounded-md shadow-sm focus:ring-[#f8c2eb] focus:border-[#f8c2eb] sm:text-sm">
                    </div>
                ...
                <div class="bg-white shadow rounded-lg p-6">
                    ...
                    <textarea name="content" id="content" rows="20" required
                              ...
                              placeholder="Start writing your post content here...">{{ old('content', $post->content) }}</textarea>
                    @error('content')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="bg-white shadow rounded-lg p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="mb-4">
                            ...
                            @if($post->image_path)
                                <img src="{{ asset('uploads/posts/' . $post->image_path) }}?v={{ $post->updated_at->timestamp }}" 
                                     alt="Current" class="max-w-full h-64 object-cover rounded-lg border border-gray-300">
                                <p class="mt-1 text-xs text-gray-500">{{ $post->image_original_name }}</p>
                        ...
                        <div>
                            <label for="image_upload" class="block text-sm font-medium text-gray-700 mb-2">Upload New Image</label>
                ...
                <div class="bg-white shadow rounded-lg p-6" id="review_fields">
                    <details class="group" {{ old('product_name', $post->product_name) || old('brand', $post->brand) || old('rating', $post->rating) ? 'open' : '' }}>
            ...
            <div class="lg:col-span-1 space-y-6">
                        ...
                        <div class="pt-4 border-t">
                            ...
                            <div class="mt-3">
                                <a href="{{ route('posts.show', [$post->published_at->year, str_pad($post->published_at->month, 2, '0', STR_PAD_LEFT), $post->slug]) }}" 
                                   target="_blank"
                                   class="text-sm text-[#f8c2eb] hover:underline font-semibold inline-flex items-center">
                ...
                <div class="bg-white shadow rounded-lg p-6">
                    ...
                    <select name="post_type" id="post_type" required
                        ...
                        <option value="list" {{ old('post_type', $post->post_type) == 'list' ? 'selected' : '' }}>Top List</option>
                        <option value="how-to" {{ old('post_type', $post->post_type) == 'how-to' ? 'selected' : '' }}>How-to Guide</option>
                    </select>
                    @error('post_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="bg-white shadow rounded-lg p-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <label class="flex items-center">
                            <input type="checkbox" name="is_editor_pick" value="1" {{ old('is_editor_pick', $post->is_editor_pick ?? false) ? 'checked' : '' }}
                                   class="rounded border-gray-300 text-[#f8c2eb] focus:ring-[#f8c2eb]">
                ...
                <!-- SEO (Collapsible) -->
                <div class="bg-white shadow rounded-lg p-6">
                    <details class="group" {{ old('meta_title') || old('meta_description') || $post->meta_title || $post->meta_description ? 'open' : '' }}>
                        <summary class="cursor-pointer text-lg font-medium text-gray-900 mb-4 list-none">
                            <span class="flex items-center justify-between">
                        ...
                        <div class="mt-4 space-y-4 pt-4 border-t">
                            ...
                            <div>
                                ...
                                <textarea name="meta_description" id="meta_description" rows="3"
                                          class="w-full border-gray-300 rounded-md shadow-sm focus:ring-[#f8c2eb] focus:border-[#f8c2eb] sm:text-sm">{{ old('meta_description', $post->meta_description) }}</textarea>
                            </div>
                            <div>
                                <label for="meta_keywords" class="block text-sm font-medium text-gray-700 mb-1">Keywords</label>
                                <input type="text" name="meta_keywords" id="meta_keywords" 
                                       value="{{ is_array(old('meta_keywords', $post->meta_keywords)) ? implode(', ', old('meta_keywords', $post->meta_keywords)) : (old('meta_keywords', $post->meta_keywords)) }}"
                                       class="w-full border-gray-300 rounded-md shadow-sm focus:ring-[#f8c2eb] focus:border-[#f8c2eb] sm:text-sm"
