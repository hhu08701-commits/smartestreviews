...
@section('content')
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold text-gray-900">Edit Affiliate Link</h1>
        <div class="flex items-center space-x-3">
            <a href="{{ route('admin.affiliate-links.show', $affiliateLink) }}" class="btn-secondary">View</a>
    ...
    <form action="{{ route('admin.affiliate-links.update', $affiliateLink) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <input type="text" name="label" id="label" 
                           ...
                           value="{{ old('label', $affiliateLink->label) }}" required>
                    @error('label')
                ...
                <div>
                    ...
                    <input type="url" name="url" id="url" 
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" 
                           value="{{ old('url', $affiliateLink->url) }}" required>
                    @error('url')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="merchant" id="merchant" 
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" 
                           value="{{ old('merchant', $affiliateLink->merchant) }}">
                    @error('merchant')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="rel" id="rel" 
                        ...
                        <option value="sponsored nofollow" {{ old('rel', $affiliateLink->rel) == 'sponsored nofollow' ? 'selected' : '' }}>sponsored nofollow</option>
                        <option value="nofollow" {{ old('rel', $affiliateLink->rel) == 'nofollow' ? 'selected' : '' }}>nofollow</option>
                        <option value="sponsored" {{ old('rel', $affiliateLink->rel) == 'sponsored' ? 'selected' : '' }}>sponsored</option>
                    </select>
                    @error('rel')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="post_id" id="post_id" 
                        ...
                        @foreach($posts as $post)
                            <option value="{{ $post->id }}" {{ old('post_id', $affiliateLink->post_id) == $post->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <textarea name="utm_params" id="utm_params" rows="3" 
                              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                              placeholder="utm_source=smartestreviews&utm_medium=affiliate&utm_campaign=product_review">{{ old('utm_params', $affiliateLink->utm_params) }}</textarea>
                    @error('utm_params')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="enabled" id="enabled" value="1" 
                           ...
                           {{ old('enabled', $affiliateLink->enabled) ? 'checked' : '' }}>
